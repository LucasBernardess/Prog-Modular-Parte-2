...
public class SondaAltaCapacidade extends Sonda{
    ...
    public SondaAltaCapacidade(Robo robo, Terreno terreno){
        super(robo, terreno);
    }
    ...
    public void prospectaHelio() {
        ...
        long segundos = Math.round(terreno.getCelulaNaPosicao(robo.getPosicaoRoboNoEixoY(),
                robo.getPosicaoRoboNoEixoX()).getConcentracaoDeHelio3() * 2.5); /// AUMENTO 150%
        if (segundos == 0) {
            segundos = 1;
        }
        ...
        robo.setHelioColetadoPeloRobo(robo.getHelioColetadoPeloRobo() + (concentracaoDeHelio * 3)); /// TRIPLICADO
